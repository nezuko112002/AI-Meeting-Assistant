...
export function findTruckingWebsiteProjects(knowledge = [], { limit = 2, excludeNames = [] } = {}) {
  ...
  return knowledge
    .filter(row => {
      ...
      const blob = [row.scopeOfServices, row.projectSummary, row.projectName].join(' ')
      return truckingRelevanceScore(row) > 0 && WEB_SCOPE_SIGNALS.test(blob)
    })
...
export function speakableScope(row) {
  if (row.projectSummary?.trim()) {
    const first = row.projectSummary.trim().split(/[.!?]/)[0].trim()
    if (first.length > 12) {
...
export function buildPortfolioNamesLine(projects = []) {
  const names = projects.map(p => p.companyName).filter(Boolean).slice(0, 3)
  if (names.length >= 2) {
    return `Recently ${names.slice(0, -1).join(', ')} and ${names[names.length - 1]} — trucking and logistics clients we built websites for.`
  }
  if (names.length === 1) {
    return `Recently ${names[0]} — a trucking/logistics client we built a website for.`
  }
  return ''
}
...
export function buildPortfolioDetailsLine(projects = []) {
  ...
  return picks
    .map(p => {
      const detail = p.projectSummary?.trim()
        ? p.projectSummary.trim().split(/[.!?]/)[0].trim()
        : speakableScope(p)
      const short = detail.length > 100 ? `${detail.slice(0, 97)}...` : detail
      return `For ${p.companyName}: ${short}.`
    })
    .join(' ')
}
...
export function pickPortfolioProjects(knowledge = [], options = {}) {
  ...
  if (citedNames.length) {
    const citedRows = citedNames
      ...
      .filter(Boolean)
    if (citedRows.length) return citedRows.slice(0, limit)
  }
  ...
  return findIndustryProjects(knowledge, 'logistics', limit)
}
